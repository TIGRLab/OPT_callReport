---
title: "OPTIMUM-Neuro conference call 2019-07-19"
...
#read in data
  df <- read.csv('/home/gherman/Downloads/OPTIMUMMainDatabaseF_DATA_2019-07-18_1516.csv', na.strings=c("","NA")) #contains all but MRI
  mri <- read.csv('/home/gherman/Downloads/OPTIMUMMainDatabaseF_DATA_2019-07-18_1515.csv', na.strings=c("","NA")) #contains just MRI
...
fu_due_list <- recruit_df%>% select(record_id,mri_fu_due, np_fu_due, bld_fu_due, mri_fu_7_mos, np_fu_7_mos,bld_fu_7_mos, enroll_mri_init,mri_fu_mth) %>% filter(!is.na(mri_fu_due) | !is.na( np_fu_due) | !is.na( bld_fu_due)| !is.na(mri_fu_7_mos)| !is.na(np_fu_7_mos) | !is.na(bld_fu_7_mos))
...
Here, the % column indicates the percentage of participants who have reached the six-month mark of the study who have received their 6 month follow-up assessments.
...
kable(fu_table, "html", escape = F, align = c('l', 'c', 'c', 'c', 'c', 'c', 'c')) %>%
  ...
  add_footnote(c(
     ...
     "`MRI` counts all participants who completed the T1 follow-up."), 
               notation = "number") %>%
  footnote(general = "Current month captures calendar month. The `%` columns calculate percentage of those due for follow-up who were scanned.")
```
