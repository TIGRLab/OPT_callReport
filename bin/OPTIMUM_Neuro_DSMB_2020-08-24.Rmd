---
title: "OPTIMUM-Neuro conference call 2020-08-24"
...
source('functions.R')
#read in data
  mri <- read.csv('../sources/OPTIMUM_2020-08-28_Neuro.csv',na.strings=c("","NA")) #contains all but MRI
  df <- read.csv('../sources/OPTIMUM_2020-08-28_Demo.csv', na.strings=c("","NA")) #contains just MRI
...
#add column that indicates if enrolled (where enrollment is 2 of 3 mri, blood, neuropsych)
  enroll_df <- prepare_enroll_df(recruit_df, enroll_criteria = 1) #make a new dataframe (did this because want new DF to consider only baseline)
